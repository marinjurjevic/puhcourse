...
> lastTwoPlus100' xs ys = addThree (last xs) (last ys) 100 
...
> freqFilter n xs = map fst $ filter f2 $ map (`f` xs) xs
>    where f x xs = (x, freq x xs)
>          f2 t = snd t >= n   

 freqFilter :: Eq a => Int -> [a] -> [a]
 freqFilter n xs = map fst $ filter ( (>= n) . snd ) $ map ( `f` xs) xs
   where f x xs = (x, freq x xs)  
